...
func (s *Server) rewriteBody(req *http.Request, contentType string, body []byte) []byte {
	...
	if s.cfg.RewriteHook == nil {
		return rewritten
	}
	return s.cfg.RewriteHook(s, RewriteEvent{
		Request:       req,
		LocalPort:     localPortFromHost(req.Host),
...
func (s *Server) rewriteURLValue(req *http.Request, raw string) (string, bool) {
	...
	return s.localURLFor(port, u), true
}
